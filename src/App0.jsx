...
function TeacherProfile({ teacher, onBack }) {
  return (
    <div style={{maxWidth:700,margin:"0 auto",padding:"32px 20px"}}>
      <button onClick={onBack} style={{background:"none",border:"none",color:"#63dcb4",cursor:"pointer",fontSize:14,marginBottom:22,display:"flex",alignItems:"center",gap:6}}>← Retour</button>
      <div style={{background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:20,padding:28}}>
        <div style={{display:"flex",gap:18,alignItems:"flex-start",marginBottom:22}}>
          <Avatar name={teacher.nom} size={68} />
          <div>
            <h2 style={{margin:"0 0 4px",fontSize:20,fontFamily:"Playfair Display,serif"}}>{teacher.nom}</h2>
            <p style={{margin:"0 0 6px",color:"#5a6370",fontSize:13}}>{teacher.departement} — USTHB</p>
            <span style={{fontSize:11,fontWeight:700,color:getGradeColor(teacher.grade),background:"rgba(255,255,255,0.06)",padding:"3px 10px",borderRadius:6}}>{teacher.grade}</span>
          </div>
        </div>
        ...
        <div style={{display:"flex",gap:20,paddingTop:18,borderTop:"1px solid rgba(255,255,255,0.07)"}}>
          <div><span style={{fontSize:20,fontWeight:800,color:"#63dcb4"}}>{teacher.projets}</span><br /><span style={{fontSize:10,color:"#5a6370",letterSpacing:1}}>PROJETS</span></div>
          <div style={{display:"flex",alignItems:"center"}}><span style={{fontSize:13,color:"#aab4c2"}}>✉ {teacher.email}</span></div>
        </div>
      </div>
    </div>
  );
...
export default function App() {
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#070f1d", color:"#e8edf3", fontFamily:"DM Sans, Segoe UI, sans-serif",
      ...
      {page==="enseignants" && selectedTeacher && <TeacherProfile teacher={selectedTeacher} onBack={()=>setSelectedTeacher(null)} />}
